...
## payment_offset
Specifies the business day offset from the period end, if the ```payment_type="In Arrears"```, or the period start, if the ```pythonpayment_type="In Advance"``` before the payment is made. ```payment_offset``` can take any integer value (both positive and negative values). A positive value for ```payment_offset``` indicates a later payment (a delay), while a negative number indicates an earlier payment. 
